...
This example renders a Ruby-authored scene through ruby.wasm and the JavaScript three.js renderer. It uses a `PlaneGeometry` backdrop, `SphereGeometry`, grouped child meshes, `MeshLambertMaterial`, `MeshNormalMaterial`, `OrbitControls`, and a material color update in the animation loop.
...
The smoke test serves the repository root, waits for the Ruby scene to reach `Running`, samples the WebGL canvas for nonblank pixels, verifies grouped meshes, `PlaneGeometry`, `SphereGeometry`, `MeshLambertMaterial`, `MeshNormalMaterial`, and `OrbitControls`, then confirms that material changes and camera drag controls reach the renderer.
