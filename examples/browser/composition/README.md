...
This example renders a Ruby-authored scene through ruby.wasm and the JavaScript three.js renderer. It uses a `PlaneGeometry` backdrop, `SphereGeometry`, grouped child meshes, `TextureLoader` repeat/wrap/filter settings, `MeshLambertMaterial`, `MeshStandardMaterial`, `MeshNormalMaterial`, `OrbitControls`, and a material color update in the animation loop.
