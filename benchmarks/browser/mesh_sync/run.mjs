...
async function main() {
  ...
  try {
    ...
    await page.goto(`${server.url}/benchmarks/browser/mesh_sync/`, { waitUntil: "load" });
    await page.waitForFunction(() => Boolean(globalThis.__threeRbMeshSyncBenchmarkJson), null, { timeout: 180_000 });
    await page.waitForTimeout(500);

    assertNonBlankCanvas(await sampleCanvas(page));
    assertNoDiagnostics(diagnostics);

    const result = JSON.parse(await page.evaluate(() => globalThis.__threeRbMeshSyncBenchmarkJson));
...
function validateResult(result) {
  if (result.benchmark !== "browser_mesh_sync" || result.mesh_count !== 1000 || result.iterations <= 0) {
    throw new Error(`unexpected benchmark metadata: ${JSON.stringify(result)}`);
  }

  for (const key of ["initial_sync_ms", "clean_sync", "dirty_transform_sync", "backend_handle_count"]) {
    if (result[key] === undefined || result[key] === null) {
      throw new Error(`benchmark result is missing ${key}: ${JSON.stringify(result)}`);
    }
  }

  if (result.backend_handle_count < 1003 || result.first_mesh_type !== "Mesh") {
    throw new Error(`benchmark did not materialize the expected scene graph: ${JSON.stringify(result)}`);
  }

  for (const section of [result.clean_sync, result.dirty_transform_sync]) {
