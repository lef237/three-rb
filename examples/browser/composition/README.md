...
This example renders a Ruby-authored scene through ruby.wasm and the JavaScript three.js renderer. It uses ambient, directional, point, and hemisphere lights, a `PlaneGeometry` backdrop, `SphereGeometry`, grouped child meshes, `TextureLoader` repeat/wrap/filter settings, `MeshLambertMaterial`, `MeshPhongMaterial`, `MeshStandardMaterial`, `MeshNormalMaterial`, `OrbitControls`, backend material/texture disposal, and a material color update in the animation loop.
