...
- `examples/browser/textures` focuses on `TextureLoader`, `RGBELoader`, repeat/wrap/filter/UV-transform settings, `MeshPhysicalMaterial` standard/physical texture maps, `MeshMatcapMaterial` matcap texture assignment, and an HDR environment texture on textured meshes.
